...
public class DIV extends Maps{
    DIV(String soure, String destination, String binarycode, String hexCode, Integer w){
        String mod = "";
        String rbym = "";
       if (destination.matches("[a-zA-Z]+")) {
           mod = ModBinaryMap.get("REG2REG");
           rbym = RegisterBinaryMap.get(destination);
           binarycode = MnemonicsToBinaryMap.get("DIV").replace("w", Integer.toString(w)).replace("mod", mod).replace("rbym", rbym);
           ...
           binarycode = MnemonicsToBinaryMap.get("DIV").replace("w", Integer.toBinaryString(w)).replace("mod", mod).replace("rbym", rbym);
           String[] binaryNibbles = binarycode.split(" ");
           for (int i = 0; i < binaryNibbles.length; i++) {
               String binaryNibble = binaryNibbles[i];
               hexCode += BinaryHexMap.get(binaryNibble);
